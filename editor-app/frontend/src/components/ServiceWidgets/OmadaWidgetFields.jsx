...
function OmadaWidgetFields({ initialData, onChange: parentOnChange }) {
  ...
  useEffect(() => {
    ...
    const errors = {};
    if (!url?.trim()) {
      errors.url = 'URL is required for Omada widget.';
    }
    if (!username?.trim()) {
    ...
    const dataForParent = { type: 'omada' };
    ...
    if (Object.keys(dataForParent).length === 0) {
        dataForParent.type = 'omada';
    }

    parentOnChange(dataForParent, errors);
  }, [url, username, password, site, parentOnChange]); // Depend on individual states and parentOnChange

  // Handle changes for standard TextFields (URL, Username, Site)
  ...
  return (
    <Box sx={{ paddingTop: 2, display: 'flex', flexDirection: 'column', gap: 2 }}>
      <TextField
        ...
        helperText={fieldErrors.url || "e.g., http://omada.example.com:8088"}
        type="url"
      />
      <TextField
        required
        ...
        helperText={fieldErrors.username || "Username for your Omada controller"}
      />
      <EnvVarAutocompleteInput
        ...
        helperText={fieldErrors.password || "Password for your Omada controller. Can be a {{HOMEPAGE_VAR_...}}"}
      />
      <TextField
        required
        fullWidth
        name="site"
        label="Site Name"
        value={site} // Use individual state
        onChange={handleTextFieldChange(setSite)} // Use specific handler with setter
        error={!!fieldErrors.site}
        helperText={fieldErrors.site || "The name of the site in your Omada controller"}
      />
    </Box>
  );
}

OmadaWidgetFields.propTypes = {
...
OmadaWidgetFields.defaultProps = {
  initialData: null,
};

export default OmadaWidgetFields;