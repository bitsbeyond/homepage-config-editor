...
function PlantitWidgetFields({ initialData, onChange: parentOnChange }) {
  ...
  useEffect(() => {
    ...
    const errors = {};
    if (!url?.trim()) {
      errors.url = 'URL is required for Plant-it widget.';
    }
    if (!apiKey?.trim()) {
    ...
    const dataForParent = { type: 'plantit' };
  ...
  return (
    <Box sx={{ paddingTop: 2, display: 'flex', flexDirection: 'column', gap: 2 }}>
      <TextField
        ...
        fullWidth
        name="url"
        label="Plant-it API URL"
        value={url} // Use individual state
        onChange={handleTextFieldChange(setUrl)} // Use specific handler with setter
        error={!!fieldErrors.url}
        helperText={fieldErrors.url || "URL of your Plant-it instance (e.g., http://plantit.local:8000 - use API port)"}
        type="url"
      />
      <EnvVarAutocompleteInput
        ...
        helperText={fieldErrors.key || "API Key created via Plant-it REST API. Can be a {{HOMEPAGE_VAR_...}}"}
      />
    </Box>
  );
}

PlantitWidgetFields.propTypes = {
...
PlantitWidgetFields.defaultProps = {
  initialData: null,
};

export default PlantitWidgetFields;