...
function OverseerrWidgetFields({ initialData, onChange: parentOnChange }) {
  ...
  useEffect(() => {
    ...
    const errors = {};
    if (!url?.trim()) {
      errors.url = 'URL is required for Overseerr widget.';
    }
    if (!apiKey?.trim()) {
    ...
    const dataForParent = { type: 'overseerr' };
  ...
  return (
    <Box sx={{ paddingTop: 2, display: 'flex', flexDirection: 'column', gap: 2 }}>
      <TextField
        ...
        helperText={fieldErrors.url || "URL of your Overseerr instance (e.g., http://overseerr.local)"}
        type="url"
      />
      <EnvVarAutocompleteInput
        ...
        helperText={fieldErrors.key || "Found in Overseerr Settings > General. Can be a {{HOMEPAGE_VAR_...}}"}
      />
    </Box>
  );
}

OverseerrWidgetFields.propTypes = {
...
OverseerrWidgetFields.defaultProps = {
  initialData: null,
};

export default OverseerrWidgetFields;