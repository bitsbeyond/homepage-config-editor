import React, { useState, lazy, Suspense, useMemo, useCallback } from 'react'; // Added lazy, Suspense, useMemo, useCallback
import {
    Button, Dialog, DialogActions, DialogContent, DialogTitle, TextField,
    FormControl, InputLabel, Select, MenuItem, Box, Typography, IconButton, CircularProgress // Added CircularProgress
} from '@mui/material';
import CloseIcon from '@mui/icons-material/Close';
import { apiRequest } from '../../utils/api'; // Import the api utility

// List of available service widget types (derived from file list, excluding index.md)
...
const serviceWidgetTypes = [
    ...
    'traefik', 'transmission', 'truenas', 'tubearchivist', 'unifi', 'unmanic', // Changed 'unifi-controller' to 'unifi'
    'uptime-kuma', 'uptimerobot', 'urbackup', 'vikunja', 'watchtower', 'wgeasy', 'whatsupdocker',
    'xteve', 'zabbix'
].filter((v, i, a) => a.indexOf(v) === i); // Ensure unique values if any duplicates were manually added elsewhere

// Helper to convert widget type string to component name (e.g., 'adguard-home' -> 'AdguardHome')
...
const widgetComponentLoaders = {
    ...
    'unifi-controller': lazy(() => import('../ServiceWidgets/UnifiWidgetFields.jsx')), // Match the type value
...
// TODO: Potentially fetch existing group names to populate dropdown?

function AddServiceForm({ open, onClose, onServiceAdded, currentServiceGroupsData = {} }) { // Changed prop name and default
    const [groupName, setGroupName] = useState('');
    const [isNewGroup, setIsNewGroup] = useState(false); // Toggle for entering a new group name
    ...
    const handleClose = () => {
        ...
        setDescription('');
        setIcon('');
        setWidgetType(''); // Reset widget type
        setWidgetData(null); // Reset widget data
        setWidgetFieldErrors({}); // Reset widget errors
        setError('');
    ...
    const handleSubmit = async (event) => {
        ...
        if (!finalGroupName && !serviceName) {
            setError('Service Name is required (Group Name is optional).');
            setLoading(false);
            return;
        }
         if (!serviceName) {
        ...
        // Construct the service object in the format: { "Service Name": { details... } }
        ...
        // Create a deep copy of the current groups data (which is an object)
        const updatedGroupsDataObject = JSON.parse(JSON.stringify(currentServiceGroupsData));

        const targetGroupName = finalGroupName || ""; // Use "" for Ungrouped
        ...
        // Add the new service to the target group
        // newServiceEntry is { "ServiceName": {details} }
        // The items in serviceGroupsData are { id: ..., name: ..., originalData: {"ServiceName":{details}}, details: {details} }
        // For saving, the API expects an array of groups, where each group is { "GroupName": [ { "ServiceName": {details} }, ... ] }
        ...
        const finalDataToSave = Object.entries(updatedGroupsDataObject).map(([groupKey, groupItemsArray]) => {
            const mappedGroupItems = groupItemsArray.map(item => {
                ...
                // A new item will be { "ServiceName": {details} } and won't have 'id' or 'originalData' at its top level.
                ...
                // Otherwise, it's a new item already in the correct format, or an item that was somehow already correct.
                return item;
            });
        ...
        console.log("Submitting updated service groups:", finalDataToSave);
        ...
        try {
            ...
            onServiceAdded(); // Notify parent component to refresh
            handleClose(); // Close dialog on success
        } catch (err) {
            console.error("Failed to add service:", err);
            setError(err.data?.error || err.message || 'Failed to add service.');
        } finally {
            setLoading(false);
        }
    };
    ...
    return (
        <Dialog open={open} onClose={handleClose} maxWidth="sm" fullWidth>
            ...
            <DialogContent>
                <Box component="form" id="add-service-form" onSubmit={handleSubmit} noValidate sx={{ mt: 1 }}>
                    ...
                    <FormControl fullWidth margin="normal" disabled={loading}>
                        ...
                        >
                            ...
                            {/* Populate with existing group names */}
                            {Object.keys(currentServiceGroupsData).map((key) => {
                                // Don't list the 'Ungrouped' option explicitly here if it's represented by an empty key
                                // or handle it based on how "Ungrouped" is stored.
                                // Assuming actual group names are non-empty strings.
                                if (key) { // Check if key is not an empty string (representing Ungrouped)
                    ...
                    {isNewGroup && (
                        <TextField
                            margin="dense" // Use dense margin when appearing conditionally
                            required
                            fullWidth
                            id="newGroupName"
                    ...
                    <TextField
                        ...
                        disabled={loading}
                        autoFocus={!isNewGroup} // Autofocus only if not creating new group
                    />
                    <TextField
                        margin="normal"
                        fullWidth
                    ...
                    <FormControl fullWidth margin="normal" disabled={loading}>
                        ...
                        <Select
                            ...
                            onChange={(e) => {
                                ...
                                // Reset widget data and errors when type changes
                                // The specific component will initialize its own defaults
                                setWidgetData(newType ? { type: newType } : null);
                                setWidgetFieldErrors({});
                            }}
                        >
                            <MenuItem value=""><em>None</em></MenuItem>
                            {serviceWidgetTypes.map((type) => (
                                <MenuItem key={type} value={type}>{widgetTypeToComponentName(type)}</MenuItem>
                            ))}
                        </Select>
                    </FormControl>
                    ...
                    {WidgetFieldsComponent && (
                        <Suspense fallback={<Box sx={{ display: 'flex', justifyContent: 'center', my: 2 }}><CircularProgress /></Box>}>
                            <WidgetFieldsComponent
                                key={widgetType} // Add key based on type
            ...
            <DialogActions sx={{ padding: '16px 24px' }}>
                ...
                <Button
                    type="submit"
                    form="add-service-form" // Link button to the form
                    variant="contained"
                    disabled={
                        loading ||
                        !serviceName ||
                ...
                >
                    {loading ? 'Adding...' : (Object.keys(widgetFieldErrors).length > 0 ? 'Fix Widget Errors' : 'Add Service')}
                </Button>
            </DialogActions>
        </Dialog>
    );
